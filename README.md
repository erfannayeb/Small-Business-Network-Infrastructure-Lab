...
|---|---|---|
| GRP-IT-Admins | john.smith, sara.jones | Local admin rights on workstations |
| GRP-Finance | fin.ali, fin.chen | Access to Finance share |
| GRP-HR | hr.mueller | Access to HR share |
...
| Workstation Security | Workstations OU | Screensaver lock 10 min, USB storage blocked, cmd.exe restricted for non-IT |
| Drive Mapping | Domain (item-level targeting) | IT → I:, Finance → F:, HR → H: mapped to department shares |
| IT Local Admin | IT OU | GRP-IT-Admins added to local Administrators group via Restricted Groups |

### File Shares and NTFS Permissions

Shared folders hosted on DC01. Share permissions set to `Everyone: Read` — access is enforced at the NTFS level.
...
**Verified:** logging in as `fin.ali` grants access to `F:` and denies access to `H:` with an Access Denied error.
...
**Verified:** connecting as `fin.ali` over VPN and successfully reaching `\\DC01\Finance`.
...
```
✓ fin.ali login → F: drive mapped, H: drive access denied
✓ john.smith login → I: drive mapped, is local admin on WS-01
✓ gpresult /r on WS-01 → all 4 GPOs listed under Applied GPOs
✓ 5 wrong password attempts → account locked out (Event ID 4740 in Security log)
✓ Screensaver activates after 10 min idle → requires domain password to unlock
✓ USB drive inserted → access blocked by GPO
✓ From DMZ VM: ping 10.0.10.x → blocked by firewall
✓ From LAN: curl http://10.0.30.x → Apache page loads
✓ OpenVPN connection as fin.ali → \\DC01\Finance accessible
```
...
│   ├── network-diagram.png
│   └── screenshots/
│       ├── 01-pfsense-firewall-rules.png
│       ├── 02-aduc-ou-structure.png
│       ├── 03-gpo-management-console.png
│       ├── 04-gpresult-output.png
│       ├── 05-fin-ali-drive-access.png
│       ├── 06-hr-access-denied.png
│       └── 07-event-4740-lockout.png
├── configs/
│   ├── pfsense-config.xml          # exported pfSense configuration
│   └── gpo-reports/
